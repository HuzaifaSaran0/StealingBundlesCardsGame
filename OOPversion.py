...
class CardsGame:
    ...
    # Method to randomly select and display cards on the screen
    ...
    def other_players_match(self):
        other_list = []
        for i in self.other_players_card:
            if i:
                for j in i:
                    other_list.append(j)
            else:
    ...
    def Match(self, thrown, index):
        ...
        if thrown in self.players[index - 1]:
            # card_thrown = True
            other_player_list = self.other_players_match()
